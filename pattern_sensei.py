...
# ─────────────────────────────────────────────────────────────────────────────
# ROBUST PATTERN GENERATOR (No Assertions - Won't Crash)
# ─────────────────────────────────────────────────────────────────────────────

def generate_pattern(pattern_type):
    """Generate patterns safely without assertions"""
    df = generate_base_candles(20)
    idx = -2  # Pattern forms at 2nd to last candle
    
    body_size = random.uniform(0.4, 0.9)
    base_price = df.iloc[idx]['open']
    
    if pattern_type == "Hammer":
        # Hammer: Real body at top, long LOWER shadow only
        open_price = base_price
        close_price = base_price + body_size  # Green body
        high_price = max(open_price, close_price) + (body_size * 0.08)  # Tiny upper wick
        low_price = min(open_price, close_price) - (body_size * 3)  # Long lower wick
        
        df.at[df.index[idx], 'open'] = open_price
        df.at[df.index[idx], 'close'] = close_price
        df.at[df.index[idx], 'high'] = high_price
        df.at[df.index[idx], 'low'] = low_price
    
    elif pattern_type == "Shooting Star":
        # Shooting Star: Real body at bottom, long UPPER shadow only
        open_price = base_price + body_size
        close_price = base_price  # Red body
        high_price = max(open_price, close_price) + (body_size * 3)  # Long upper wick
        low_price = min(open_price, close_price) - (body_size * 0.08)  # Tiny lower wick
        ...
        # Doji: Open ≈ Close, long wicks both directions
        open_price = base_price
        close_price = base_price + random.uniform(-0.03, 0.03)  # Very small difference
        high_price = base_price + (body_size * 2.5)
        low_price = base_price - (body_size * 2.5)
        ...
        prev_open = base_price + body_size * 0.3
        prev_close = base_price - body_size * 0.3
        df.at[df.index[prev_idx], 'open'] = prev_open
        df.at[df.index[prev_idx], 'close'] = prev_close
        df.at[df.index[prev_idx], 'high'] = max(prev_open, prev_close) + body_size * 0.1
        df.at[df.index[prev_idx], 'low'] = min(prev_open, prev_close) - body_size * 0.1
        
        # Current: big green engulfs previous body
        curr_open = base_price - body_size * 0.7
        curr_close = base_price + body_size * 1.5
        df.at[df.index[idx], 'open'] = curr_open
        df.at[df.index[idx], 'close'] = curr_close
        df.at[df.index[idx], 'high'] = max(curr_open, curr_close) + body_size * 0.2
        df.at[df.index[idx], 'low'] = min(curr_open, curr_close) - body_size * 0.2
    ...
    # Ensure OHLC consistency (critical to prevent crashes)
    df = fix_ohlc(df)
...
def generate_base_candles(n=20):
    """Generate realistic candlestick data with proper ranges"""
    np.random.seed(int(time.time()) + random.randint(0, 1000))
    start_price = random.uniform(85, 115)
    ...
    # Calculate wicks properly
    df['high'] = df[['open', 'close']].max(axis=1) + np.random.uniform(0.1, volatility*start_price*0.8, n)
    df['low'] = df[['open', 'close']].min(axis=1) - np.random.uniform(0.1, volatility*start_price*0.8, n)
    
    df['volume'] = np.random.randint(200000, 800000, n)
    df['date'] = [datetime.now() - timedelta(minutes=5*i) for i in range(n)][::-1]
    
    # Ensure proper OHLC ordering
    df = fix_ohlc(df)
...
def draw_chart(df, is_cheat_sheet=False):
    ...
    if is_cheat_sheet:
        ...
        fig.update_layout(
            title="📊 CHART ACTION - <span style='color:yellow'>YELLOW LINE shows pattern candle</span>",
    ...
    # Only add markings if df has data
    if len(df) >= 2:
        ...
        if remaining > 0:
            ...
            for i, pattern in enumerate(pattern_options):
                with cols[i % 3]:
                    if st.button(
                        pattern, 
                        key=f"pat_{pattern}_{int(time.time())}_{random.randint(0,1000)}", 
